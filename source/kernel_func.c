...
int sys_open_hook(struct thread* td, struct syscall_open* args) {
	...
	int (*original_call)(struct thread* td, void* args) = (void *)&ptrKernel[KERN_SYS_OPEN];

	int ret = -1;
	char possible_path[500];
	char* real_path = args->path;

	if (cred->cr_uid == 0 || k_strncmp(td->td_proc->p_appid, data->cusa, 4) != 0 ) {
		goto original;
	}

	k_strcpy(possible_path, data->data_path);
	k_strcpy(possible_path + k_strlen(possible_path), td->td_proc->p_appid);
	k_strcpy(possible_path + k_strlen(possible_path), args->path);
	...
	if (ret > 0) {
		...
		//printfkernel(data->print_debug1, args->path, ret);
	} else {
		printfkernel(data->print_debug2, args->path, ret);
	}
...
void install_hooks(void) {
	...
	struct hook_data* data = (struct hook_data*)rwx_kalloc(sizeof(struct hook_data));
	k_strcpy(data->data_path, "/user/data/");
	k_strcpy(data->cusa, "CUSA");
	k_strcpy(data->print_debug1, "[SYS_OPEN] Original: %s (%i)\n");
	k_strcpy(data->print_debug2, "[SYS_OPEN] Redirected: %s (%i)\n");

	data->sys_open_orig = sysents[5].sy_call;

	void* hook_ptr = (void*)rwx_kalloc(0x10000);
	k_memcpy(hook_ptr, sys_open_hook, 0x1000);

	uint64_t cr0 = readCr0();
	writeCr0(cr0 & ~X86_CR0_WP);
	*(uint64_t*)ptrKernel = data;
	sysents[5].sy_call = hook_ptr;
	writeCr0(cr0);

	struct proc* app = proc_find_by_name("SceShellCore");

	if (app) {
		int vm_nbr = 0;
		Map* vm_map = kern_process_vm_map(app, &vm_nbr);

		uint64_t nop_slide = 0x9090909090909090;

		// Mount /user on any process sandbox with read/write perm
		kern_process_readwrite_mem(app, vm_map[1].start + 0x31CA2A, &nop_slide, 6, 1);

		k_free(vm_map, M_TEMP);
	}
}
